...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            // 导入数据
            importExcel: function(formId, width, height,url) {
            	table.set();
				var _url = $.common.isEmpty(url) ? table.options.importUrl : url;
    	...
    	modal: {
            ...
            // 确认窗体 - 扩展，只有一个选项
            confirm_single: function (content, callBack) {
                layer.confirm(content, {
                    icon: 1,
                    title: "系统提示",
                    btn: ['确认']
            ...
            open: function (title, url, width, height, callback) {
                ...
                if ($.common.isEmpty(height)) {
                	//height = ($(window).height() - 50); 此处调整了一下，否则太长了
            ...
            // 弹出层指定宽度，但是没有按钮的那种
            openNotBtn: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            openFullNoOpt: function (title, url, width, height) {
				//打开没选项的全屏界面
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
        ...
        operate: {
			...
			expire: function(id) {
				table.set();
				$.modal.confirm("确定过期该条" + table.options.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? table.options.removeUrl : table.options.removeUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
			...
			//callBack 和 noConfirm 是zhuwenchao后面追加的。
            removeObject: function(url, data, callBack, needConfirm = true) {
                table.set();
                // false true
                if (needConfirm) {
			...
			expireAll: function() {
				...
				$.modal.confirm("确认要过期选中的" + rows.length + "条数据吗?", function() {
					var url = table.options.removeUrl;
					var data = { "ids": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
            ...
            // 添加信息 全屏，hasOptions表示下面的操作按钮是否要有
            addFull: function(id,hasOptionBtn=true) {
            	table.set();
            	...
            	if(hasOptionBtn){
					...
					$.modal.openFullNoOpt("添加" + table.options.modalName, url);
				}
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            // 修改信息
            edit: function(id,isFull=false,hasOptionBtn=true) {
            	table.set();
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            		...
            		if(isFull){
            			if(hasOptionBtn){
							$.modal.openFull("修改" + table.options.modalName, $.operate.editUrl.apply(this, arguments));
						}else{
							$.modal.openFullNoOpt("修改" + table.options.modalName, $.operate.editUrl.apply(this, arguments));
						}
					}else{
						$.modal.open("修改" + table.options.modalName, $.operate.editUrl.apply(this, arguments));
					}

            		// if (arguments.length > 1) {
					// 	$.modal.open("修改" + table.options.modalName, $.operate.editUrl(arguments));
					// }else {
            ...
            // 修改信息，以tab页展现
            editTabWithTitle: function(title , id) {
                table.set();
            ...
            editUrl: function(id) {
            	...
            	if ($.common.isNotEmpty(id)) {
            		if (arguments.length > 1) {
						url = table.options.updateUrl;
						for (var argument in arguments) {
							url = url.replace("{id}", arguments[argument]);
						}
					}else {
						url = table.options.updateUrl.replace("{id}", id);
					}
    	...
    	common: {
            ...
            //自动处理ajax的返回值的
			resultTodo:function(result,successCallback,failCallback,showSuccessMsg=false){
				if(['code'] in result){
					if(result.code == web_status.SUCCESS){
					    if(showSuccessMsg){
                            $.modal.confirm_single(result.msg,function(){
